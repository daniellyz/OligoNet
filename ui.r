...
shinyUI(fluidPage(
  ...
  fluidRow(
    ...
    column(4,
           ...
           checkboxGroupInput("visual", label = h6(""), choices = list("Show entire network" = 1, "Remove triangles" = 2,"Remove free amino acids" = 3, "Correlation between nodes bigger than" = 4), selected = c(1,2)),
           sliderInput("cor_min", label = h6(""), min = -1, max = 1, value = 0.8,step=0.1),
           uiOutput("Shows"),
